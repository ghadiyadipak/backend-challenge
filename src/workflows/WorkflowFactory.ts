...
interface WorkflowStep {
    taskType: string;
    stepNumber: number;
    id?: string;           // Step identifier for dependency reference
    dependsOn?: string[];  // Array of step IDs this task depends on
}
...
export class WorkflowFactory {
    ...
    async createWorkflowFromYAML(filePath: string, clientId: string, geoJson: string): Promise<Workflow> {
        ...
        const tasks: Task[] = workflowDef.steps.map(step => {
            ...
            task.stepId = step.id || `step${step.stepNumber}`;
            task.dependsOn = step.dependsOn ? step.dependsOn.join(',') : undefined;
