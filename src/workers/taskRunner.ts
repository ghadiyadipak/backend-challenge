...
export class TaskRunner {
    ...
    async run(task: Task): Promise<void> {
        ...
        if (currentWorkflow) {
            ...
            if (anyFailed) {
                ...
                // Aggregate results even on failure
                currentWorkflow.finalResult = await this.aggregateFinalResults(currentWorkflow, resultRepository);
    ...
    /**
     * Aggregates results from all tasks in the workflow into a final result JSON
     */
    private async aggregateFinalResults(
        workflow: Workflow,
        ...
        const finalResult = {
            ...
            summary: {
                ...
                completed: taskResults.filter(t => t.status === TaskStatus.Completed).length,
                failed: taskResults.filter(t => t.status === TaskStatus.Failed).length
            }
        };
